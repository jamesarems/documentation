...
From there, the ways for the CLI to recognize the key are either through the global `-k, --apiKey` and `-s, apiKeyId` options or the `SNOWPLOW_CONSOLE_API_KEY` and `SNOWPLOW_CONSOLE_API_KEY_ID` environment variables. The Snowtype CLI does automatically read from a `.env` file at the root of your project as well.
